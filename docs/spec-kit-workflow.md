...
>
> **Advanced again 2026-08-06: the range now ends at OD-28**, and it had gone
> three entries behind rather than one. **OD-26** landed 2026-08-05 and settles
> which of two artifacts holds FR-006's terminal-state taxonomy; **OD-27** and
> **OD-28** landed 2026-08-06, the first admitting an operator-declared model
> rate as a second provenance, the second keeping the `SessionTable` →
> `Repository` migration deferred and routing the shape to `tasks.md` T016's
> note. **None of the three changes feature 001's closure or this row's other
> figures** — the experiment counts and the verdict are untouched. On the same
