...
>
> **What the harness did establish, at zero cost, and the line around it.** The postcondition arm detects **all 9 numeric value errors** in the eligible population **including all 3 sub-1% near-misses**, with **zero false alarms across 220 clean positives** — *the offline full-corpus sweep; see the labelling note in §8, because the `FPR_c2 = 0/60` quoted elsewhere is the judge-scored sample and a smaller population* — through a six-rung precision ladder committed before any derivation was written that **contains no numeric constant**. Separately and as a preregistered §7(5) prediction, the *schema*-derived arm detects **0 of 9**: a schema alone does not reach value errors, because no clause of it compares a magnitude to anything. **So the mechanism is demonstrated and its necessity is unmeasured.** The one-line form worth carrying: *the verifier works; nobody knows whether it is needed.*
>
...
**Labelling note added 2026-08-03 on the verification row's false-alarm figure.** Two different populations have been quoted interchangeably across this corpus, and both are zero, so nothing written is wrong — but the denominators differ, ~~by roughly 3.7×~~ **220 against 60** *(corrected 2026-08-03; the multiplier was derived in the consuming document and appears in no finding)*, and no citation said which was which.

| Figure | Population | What produced it |
|---|---|---|
| **0 of 220** | Every clean oracle-positive in the frozen corpus: all 226 oracle-positives less the 6 stale ones the packaging defect produced | An offline **full-corpus sweep** of the postcondition arm over all 246 frozen records against the committed offline fixture. Model-free, no sampling, and not part of any run's reported metrics |
