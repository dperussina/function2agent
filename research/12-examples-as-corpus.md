...
> - **Eight repos, not nine.** The directive named nine; `examples/` contains eight. Measured: **1,109,021 lines of source** across **4,420 source files** in the counted languages, plus ~212k lines of Markdown, ~100k lines of notebook JSON, and one 165k-line SQL data dump. **⚠️ Scope superseded 2026-08-10: `examples/` now holds nine directories, the ninth being `labs-OO-Agents`, so every figure in this document is a figure over the eight rather than over `examples/` — see §1.**
...
> **⚠️ THE SCOPE CLAIM IS SUPERSEDED 2026-08-10, and the count sentences before it are not.** *"`ls examples/` returns eight directories"* is a dated observation of what this pass saw on 2026-08-02 and stands as written. **What no longer holds is the scope: `examples/` now holds nine directories, so "those eight" is a subset and this document reads as though it were the whole.** The ninth is `labs-OO-Agents`, and it is **not** the `google/genai-toolbox` predicted above — that candidate is still not vendored, so the open note asking for a ninth to be *added deliberately rather than assumed* was never closed. A ninth arrived by a different route instead.
>
> **What this does to the document's coverage, stated so it is not read as a digit fix.** Every corpus-wide figure below is a figure **over the eight named in §2.1** and not a figure over `examples/`: the source-line and file totals in the TL;DR, the language mix and the absent-language claims in §4, and the `codegraph index` smoke test in §5, which proposes indexing "all eight repos". `codegraph` is one of the eight and always was — it is row 5 of §2.1's table — so nothing in this document was displaced; a repository was **added** beside it. Whether the ninth moves any of those aggregates is unmeasured here.
>
> **The gap is documented, and it is documented elsewhere.** `labs-OO-Agents` is assessed on its own in [`15-nvidia-oo-agents.md`](./15-nvidia-oo-agents.md), so the corpus is not blind to it — **this document is.** Extending this document to cover the ninth is a research task and is deliberately not taken here; what is taken is the notice that its scope is narrower than its section headings imply.
...
- **`src/db/schema.sql`** — ~~four tables~~ **twelve tables**. Per `06`, the SQLite file is the most important extension point for a polyglot product: queryable from Python, Go, anything. **Design against the schema, not the TypeScript API.** **Struck 2026-08-10, and the "Per `06`" survives the correction because it was accurate: `06`'s codegraph extension-points table says "Four tables" itself, so this is an inherited figure and not a compression introduced here. Measured from the 194-line file: 7 ordinary tables, `nodes_fts`, and the 4 shadow tables its FTS5 declaration materialises — the 12 `type='table'` rows `src/analysis/codegraph_pin.py` admits — with 20 indexes and 3 triggers on the same filter, 35 objects, all of them in the digest it pins. **Those figures are the pin's population, not `sqlite_master`'s: raw the table holds 13 tables, 23 indexes and 39 objects, the difference being `sqlite_sequence` — materialised by two `INTEGER PRIMARY KEY AUTOINCREMENT` declarations and excluded by the name clause alone, since it carries non-null SQL — plus three implicit `sqlite_autoindex_*` entries. Hand-counting gives 13 and does not contradict the 12.** D-14 does not move: designing against the schema rests on the file being documented and language-neutral, not on its width, and a reader sizing that work should size it against 7 tables they will actually query rather than against either published count.**
