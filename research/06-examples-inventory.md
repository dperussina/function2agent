...
| **The SQLite file itself** | `.codegraph/*.db`. ~~Four tables~~ **twelve tables**, documented schema, stable IDs. **You can query it from Python, Go, anything.** This is the most important extension point for a polyglot product — you are not locked to the TS API. **Struck 2026-08-10, and this site is the origin rather than a copy of one: `12` §6.5 and `14` §2.2 both say four and both cite this document, so the figure was inherited faithfully and was wrong here first. Measured from the 194 lines of `src/db/schema.sql` that this section reports reading in full — 7 ordinary tables, plus `nodes_fts` and the 4 shadow tables SQLite materialises from its FTS5 declaration, which together are the 12 `type='table'` rows of `sqlite_master` that the pin admits — alongside 20 indexes and 3 triggers on the same basis, 35 objects in all, every one of which participates in the digest `src/analysis/codegraph_pin.py` pins. **Every figure in that sentence counts the pin's population and not `sqlite_master`'s, which matters because the two differ: counted raw the table holds 13 tables, 23 indexes and 39 objects.** The extras are `sqlite_sequence`, which SQLite materialises because `edges` and `unresolved_refs` declare `INTEGER PRIMARY KEY AUTOINCREMENT`, and three implicit `sqlite_autoindex_*` indexes. `schema_digest()` selects `WHERE sql IS NOT NULL AND name NOT LIKE 'sqlite_%'`, and the two predicates do not do equal work — the autoindexes carry no SQL and would fall to either, but `sqlite_sequence` carries non-null SQL, so the name filter alone is what excludes it and the 12 rests on that clause. **So 12 is what the pin asserts, and a reader who counts the rows by hand gets 13 and has found the filter rather than an error here.** One place still carries the total wrongly and cannot be repaired: commit `a14a042`'s message reads *"Materialised, that is 12 rows of `sqlite_master` with type `table`"*, which is false as a total, is pushed, and is not editable in place; this paragraph is the record that it is wrong, on the same footing as the `acdf5f7` record in `tools/README.md`. Commit `b2c129f`'s message is a second on the same footing and is the origin rather than a copy of one: it reads *"it answers 14 rows to `sqlite_master`'s 13 with two internal objects typed `table` rather than one"*, which is unscoped, sets a row total against a `type='table'` count, and says two where three come back typed `table`. The docstring it wrote carried that sentence and was corrected in place on 2026-08-10; the message cannot be. Note what the correction does not touch: the claim this row exists to make is that the file is documented and queryable from any language, and that never depended on how many tables it has.** Appended 2026-08-10: **`PRAGMA table_list` is the method a reader reaches for next to separate those categories by type rather than by name, and it is not a substitute for counting `sqlite_master`.** Say first what it is good for, because it is genuinely better than the alternative at one thing: it types `nodes_fts` as `virtual` and the four shadows as `shadow`, which name-shape matching only approximates — the shadow names are the virtual table's name plus a suffix by convention, not by guarantee. What it cannot do is the separation actually in question. `sqlite_sequence` comes back `type='table'`, indistinguishable *by type* from the seven ordinary tables, so a decomposition drawn from the pragma assumes the category it was invoked to establish. **And it over-answers: scoped to `schema='main'` it returns 14 rows against `sqlite_master`'s 13 `type='table'` rows, the extra being `sqlite_schema` — which is not a `sqlite_master` row at all — so two of its `table`-typed objects are SQLite's own rather than this schema's.** Two cautions on the shape of that 14, both of which a re-deriving reader will hit. It is a **row total across all types**, not a count of `type='table'` rows, which is 9 in `main` and 10 overall. And it is schema-scoped: run bare, the pragma returns **15** rows, because `temp` contributes `sqlite_temp_schema`, typed `table`, on a connection that has created no temporary object — a third internal object typed `table` and a second one absent from `sqlite_master`. So the reconciliation is `sqlite_master` 13, pin-filtered 12, `table_list` 14 in `main` and 15 raw, with **no type in the pragma's own output explaining either gap** — which is the trap: the method that looks most principled is the one that produces the discrepancy. Measured against the committed `tests/fixtures/codegraph-schema/schema.sql` on CPython 3.12.11's `sqlite3` at SQLite **3.53.3**, Darwin/arm64, euid 501; a pragma's output set is version-sensitive, so the version is part of the claim rather than context for it. |
...
- `codegraph/src/db/schema.sql` (194 lines, read in full) — nodes/edges/files/unresolved_refs, FTS5, `name_segment_vocab`, indexes, `project_metadata`, **`schema_versions` — added 2026-08-10, having been omitted when this line was written; it is the seventh ordinary table, and its absence is why this enumeration could sit beside §1's "Four tables" without contradicting it loudly enough to be noticed**
