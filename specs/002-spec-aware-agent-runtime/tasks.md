...
2026-08-04** — the preflight's real-`unshare` pair, which is neither deferred nor done)* · **Estimate**: derived for
...
- [ ] T206 [P] Extend the preflight's `namespaces` check to **attempt a real `unshare(CLONE_NEWUSER)` in a forked child, and an `unshare(0)` no-op beside it**, reporting which layer refused and what the operator can do about it, in `src/supervisor/preflight.py` (FR-048, **OD-17**, FR-053)
