...
- [ ] T101 [US1] **Measure the syscall supervisor's overhead on the reference application before the mechanism is committed**, recording the figure and the shell-heavy arm that stresses it, in `tests/batteries/test_seccomp_overhead.py` (**Q-09** — accepted *with* the measurement, not with a prediction of its result; if it is prohibitive the recorded fallback is an audit channel that keeps SC-022 and loses the before-execution property)
  ...
  - **Two stale claims in the committed measurement artifact corrected.** `what_this_is_a_property_of` said "the reference application does not exist yet", true when written on 2026-08-03 and false since T116 landed on 2026-08-08. It also described "these three workloads"; there are five. Added beside them: the interpreter start every arm pays in **both** its baseline and its supervised run, which cancels out of `overhead_seconds` and inflates `notifications_observed` — so `microseconds_per_notification` is the transferable figure and `ratio` is not.
  - **No removal proof names an assertion inside `test_seccomp_overhead.py`, deliberately.** That module is `linux_only` and `privileged`, so on any host that cannot run it every such proof reports SKIPPED — an outcome that costs a reader the same attention as a real one and carries none. The two T101 proofs name tests that run everywhere: the workload-rot guard and the process-spawn enumeration.
