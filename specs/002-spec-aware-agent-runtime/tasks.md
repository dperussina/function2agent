...
- [X] T206 [P] Extend the preflight's `namespaces` check to **attempt a real `unshare(CLONE_NEWUSER)` in a forked child, and an `unshare(0)` no-op beside it**, reporting which layer refused and what the operator can do about it, in `src/supervisor/preflight.py` (FR-048, **OD-17**, FR-053)
  ...
  - ⚠️ **The mechanism is untested against a real refusal.** Every cell in `tests/unit/test_namespace_probe.py` is constructed by injecting the attempt function; nothing in the suite has observed a genuine `EPERM` from a seccomp profile. The one test that touches the kernel asserts only that the probe does not move the process that ran it, and it skips off Linux. The classification is **derived from [finding 024](./findings/024-deployment-surface-permission-census.md)'s measured table, not re-measured here** — and its LSM branch was derived there too, since that host carries no AppArmor and no SELinux
